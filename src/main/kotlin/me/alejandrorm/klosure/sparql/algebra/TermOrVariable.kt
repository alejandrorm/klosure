...
sealed class TermOrVariable {
    ...
    class QuotedTriple(val subject: TermOrVariable, val predicate: TermOrVariable, val obj: TermOrVariable): TermOrVariable() {
        ...
        override fun match(solution: SolutionMapping, tripleId: NodeId): SolutionMapping? {
            if (tripleId is TripleId) {
                // could resolve before calling match, but it seems redundant, matches are going fail in the
                // variable.match check
                val s1 = subject.match(solution, tripleId.subject)
                val s2 = s1?.let { predicate.match(s1, IriId(tripleId.predicate)) }
                return s2?.let { obj.match(s2, tripleId.obj) }
            }
            return null
        }
