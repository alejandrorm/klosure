...
fun getEffectiveBooleanValue(node: Node?): BooleanLiteral? {
    ...
    return when (node) {
        is IntegerLiteral -> if (node.value != 0L) TrueLiteral else FalseLiteral
        is DoubleLiteral -> if (!node.value.isNaN() && node.value != 0.0) TrueLiteral else FalseLiteral
        is BigDecimalLiteral -> if (node.value.toDouble() != 0.0) TrueLiteral else FalseLiteral
        is BigIntegerLiteral -> if (node.value.toLong() != 0L) TrueLiteral else FalseLiteral
        is StringLiteral ->
